<template>
  <div class="min-h-screen bg-white dark:bg-zinc-900 flex items-center justify-center px-4">
    <div class="max-w-2xl w-full text-center">
      <!-- Error Message -->
      <div class="mb-8">
        <h1 class="text-6xl font-bold text-zinc-900 dark:text-zinc-100 mb-4 font-mono">
      ...
      <div v-if="relatedPosts.length > 0" class="mb-8">
        ...
        <div class="space-y-3 text-left">
          ...
          >
            <h4 class="font-semibold text-zinc-900 dark:text-zinc-100 mb-1" v-html="highlightMatches(post.title, post.matchedTerms)">
            </h4>
            <p class="text-sm text-zinc-600 dark:text-zinc-400">
              {{ getPostType(post.slug) }} • {{ formatDate(post.date) }}
            </p>
            <p class="text-xs text-zinc-500 dark:text-zinc-500 mt-1">
              Matches: <span class="font-medium">{{ post.matchedTerms.join(', ') }}</span>
            </p>
          </NuxtLink>
        </div>
      </div>
...
const highlightMatches = (title, matchedTerms) => {
  ...
  matchedTerms.forEach(term => {
    ...
    highlighted = highlighted.replace(regex, '<strong class="bg-yellow-200 dark:bg-yellow-800 px-1 rounded">$1</strong>')
  })
  return highlighted
}
