  ...
  <div v-if="hasCommits" ref="contributionsRef">
    ...
    <div class="mini-calendar">
      <div class="contribution-grid">
        <div v-for="(week, weekIndex) in contributionWeeks" :key="`week-${weekIndex}`" class="week-column">
          <div
            v-for="(day, dayIndex) in week" :key="`day-${weekIndex}-${dayIndex}`" class="contribution-square" :class="{
...
const firstDate = computed(() => {
  ...
  if (!firstWeek?.length || !firstWeek[0]?.date) return ''
...
const lastDate = computed(() => {
  ...
  if (!lastWeek?.length || !lastWeek[6]?.date) return ''
  return format(new Date(lastWeek[6].date), 'MMM d')
})

// Template ref used for potential future animations (currently no-op)
