...
const viewBox = computed(() =>
  asset.value ? handDrawnViewBox(asset.value) : '0 0 1 1'
)
...
const dims = computed(() =>
  props.stretch
    ...
    : {
        ...
        width: `calc(${heightCss.value} * ${ratio.value})`,
      }
)
...
<template>
  ...
  <span
    v-else
    class="hand-drawn-missing"
    :title="`unknown hand-drawn asset: ${name}`"
  >
    ⟨{{ name }}?⟩
  </span>
</template>
...
.hand-drawn {
  ...
  /* MUST clip: <use> pulls the whole sprite; the viewBox crop only shows
     through
