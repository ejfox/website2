<template>
  <article class="commit-card group">
    <div class="flex gap-3">
      <!-- Git icon -->
      <div class="flex-shrink-0 w-10 h-10 rounded-full bg-green-50 dark:bg-green-950 flex items-center justify-center">
      ...
      <div class="flex-grow min-w-0">
        ...
        <div class="flex items-center gap-2 mb-1">
          <span class="font-medium text-zinc-900 dark:text-zinc-100 text-sm">{{ repo }}</span>
          <span class="text-zinc-300 dark:text-zinc-700">·</span>
          <code class="text-xs font-mono text-zinc-400 dark:text-zinc-500">{{ sha }}</code>
        </div>

        <!-- Commit message -->
        <p class="text-zinc-700 dark:text-zinc-300 text-[15px] leading-relaxed font-mono">{{ message }}</p>

        <!-- Footer -->
        <div class="flex items-center gap-4 mt-2 text-xs text-zinc-400 dark:text-zinc-500">
