  ...
  <div v-if="item" class="card">
    <div class="card-top">
      <div class="card-identity">
        <NuxtLink :to="`/gear/${item.slug}`" class="card-name">{{ item.Name }}</NuxtLink>
        <span v-if="item.Brand" class="card-brand">{{ item.Brand }}</span>
      </div>
      <div class="card-weight" :title="`${item.Weight_oz}oz`">
        {{ weightGrams }}<span class="card-weight-unit">g</span>
      </div>
    </div>
    ...
    <div class="card-footer">
      ...
      <span v-if="item.Condition" class="card-tag" :class="conditionColor">{{ item.Condition }}</span>
      <NuxtLink :to="`/gear/${item.slug}`" class="card-cta">open in gear inventory →</NuxtLink>
    </div>
  </div>
...
const weightGrams = computed(() => {
  const oz = parseFloat(item.value?.Weight_oz ?? '')
  if (!oz || oz <= 0) return '—'
...
const conditionColor = computed(() => {
  const map: Record<string, string> = {
    New: 'tag-bright', Good: 'tag-mid', Fair: 'tag-dim', Poor: 'tag-faint',
  }
...
.tag-bright { @apply text-zinc-700 dark:text-zinc-300 border-zinc-300 dark:border-zinc-600; }
.tag-mid    { @apply text-zinc-500 dark:text-zinc-500; }
.tag-dim    { @apply text-zinc-400 dark:text-zinc-600; }
.tag-faint  { @apply text-zinc-300 dark:text-zinc-700; }
...
/* Skeleton */
.card-skeleton { @apply animate-pulse; }
.skeleton-name   { @apply h-4 w-1/2 rounded bg-zinc-200 dark:bg-zinc-800 mb-2; }
.skeleton-note   { @apply h-3 w-full rounded bg-zinc-200 dark:bg-zinc-800 mb-1; }
.skeleton-footer { @apply h-3 w-1/3 rounded bg-zinc-200 dark:bg-zinc-800; }

/* Error */
.card-error  { @apply opacity-40; }
.error-label { @apply text-2xs text-zinc-500; }
</style>
