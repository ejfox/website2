<template>
  ...
  >
    ...
    <div v-if="gearItem.amazon" class="text-center mt-6">
      <a
        :href="amazonUrl" target="_blank" rel="nofollow noopener"
    ...
    <div class="mt-8 border-t border-zinc-200 dark:border-zinc-700 pt-6">
      <h3 class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-4 uppercase tracking-wider">
        Item Details
      </h3>
      <div class="grid grid-cols-1 gap-2 text-xs">
        <div
          v-for="(value, key) in itemDetails" :key="key"
        ...
        >
          ...
          <span class="text-zinc-900 dark:text-zinc-100 font-mono text-right truncate ml-2 min-w-0" :title="value">{{
            value || '—' }}</span>
        </div>
      </div>
    </div>
...
.gear-card {
  ...
  transform:
    perspective(1500px) rotateX(-90deg) rotateY(45deg) rotateZ(-20deg) scale(0.4) translateZ(-200px);
