<template>
  <div ref="container" class="viewer" :style="{ height }">
    ...
    <div v-if="status !== 'ready'" class="overlay" role="status" aria-live="polite">
      <div v-if="status === 'loading'" class="spinner" aria-label="Loading 3D model" />
...
const { modelUrl, autoRotate = true, height = '300px' } = defineProps({
...
onMounted(() => nextTick(() => {
  if (!canvas.value) { status.value = 'error'; return }
...
onBeforeUnmount(() => { cancelAnimationFrame(raf); controls?.dispose(); renderer?.dispose() })
</script>

<style scoped>
.viewer  { @apply relative w-full overflow-hidden bg-[#0a0a0a]; }
.overlay { @apply absolute inset-0 flex items-center justify-center pointer-events-none; }
...
.error-label { @apply font-mono text-3xs text-cyan-500/50 tracking-[.1em]; }
