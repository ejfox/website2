<template>
  ...
  >
    ...
    <div
      class="grid grid-cols-12 gap-3 py-2.5 px-2 text-xs font-mono items-center border-b border-zinc-200/60 dark:border-zinc-800/40"
    >
      ...
      <div class="col-span-6 truncate" :title="item.Name">
        <div class="flex items-center gap-2">
          ...
          <div
            v-if="scores.M >= 5"
            class="flex gap-0.5 opacity-40 group-hover:opacity-70 transition-opacity"
          >
            <div
              v-for="n in multiUseDots"
              :key="n"
          ...
          >
            <span
              v-if="itemAge < 365"
              class="text-[9px] text-zinc-400 dark:text-zinc-600"
              title="New — less than 1 year old"
            >✦</span>
            <span
              v-else-if="itemAge < 730"
              class="text-[9px] text-zinc-500 dark:text-zinc-500"
              title="Broken in — 1-2 years old"
            >✧</span>
            <span
              v-else-if="itemAge < 1095"
              class="text-[9px] text-zinc-600 dark:text-zinc-400"
              title="Seasoned — 2-3 years old"
            >◈</span>
            <span
              v-else-if="itemAge < 1825"
              class="text-[9px] text-zinc-700 dark:text-zinc-300"
              title="Weathered — 3-5 years old"
            >◇</span>
            <span
              v-else
              class="text-[9px] text-zinc-800 dark:text-zinc-200"
      ...
      >
        ...
        <span
          class="hidden group-hover/type:inline text-[10px] uppercase tracking-wider font-medium"
        >{{ item.Type || '—' }}</span>
      </div>

      <!-- Waterproof - Secondary data -->
      <div
        class="col-span-2 text-center text-[10px] text-zinc-500 dark:text-zinc-500 font-medium"
      >
        {{ item.Waterproof || '—' }}
      </div>
      ...
      <div class="col-span-1 text-right">
        <div v-if="baseWeight > 0" class="flex items-center justify-end gap-1">
          ...
          ></div>
          <span
            class="tabular-nums text-zinc-900 dark:text-zinc-100 font-semibold tracking-tight"
