    ...
    <div class="specimens">
      <div v-for="item in items" :key="item.Name" class="specimen">
        <div class="specimen-image">
          <img
            v-if="item.Photo_URL?.trim()"
            :src="item.Photo_URL"
            :alt="item.Name"
            class="specimen-photo"
          />
          <span v-else class="specimen-symbol">
        ...
        <div class="specimen-labels">
          ...
          <span class="specimen-type">
            {{ item.Type || '—'
            }}{{ item.Condition ? ' · ' + item.Condition : '' }}
          </span>
          <span class="specimen-pips">
...
.plate-total {
  @apply font-mono text-3xs tabular-nums text-zinc-400;
}
...
.specimen-photo {
  @apply object-cover w-[120px] h-[120px] grayscale opacity-80;
}
.specimen-symbol {
  @apply text-4xl text-zinc-400 dark:text-zinc-600 leading-none;
}
.specimen-rule {
  @apply w-full mt-3 border-b border-zinc-300 dark:border-zinc-700;
}
