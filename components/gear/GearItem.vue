<template>
  <tr class="row group">
    ...
    <td class="cell-buy">
      <a v-if="item.Amazon_URL" :href="affiliateUrl(item.Amazon_URL)"
        target="_blank" rel="nofollow noopener"
        class="badge-orange-hover" v-tooltip="'Buy on Amazon'" @click.stop>BUY</a>
    ...
    <td class="cell">
      <span class="priority-pips">{{ PRIORITY_PIPS[item.Priority] || '—' }}</span>
    </td>
    ...
    <td class="cell-weight">
      <span class="text-3xs font-medium">{{ formatWeight(item.Weight_oz) }}</span>
    </td>
  </tr>
</template>
...
const CATEGORY_ABBR: Record<string, string> = {
  Adventure: 'ADV', Tech: 'TCH', Photo: 'PHT',
  Motorcycle: 'MTO', Home: 'HOM', Clothing: 'CLT',
}
const categoryAbbr = (c: string) => c ? (CATEGORY_ABBR[c] ?? c.substring(0, 3).toUpperCase()) : '—'

// ── Waterproof ───────────────────────────────────────────
const waterproofDot = (w: string) => ({ Yes: '●', No: '○', Partial: '◐' }[w] ?? '—')
...
// ── Age since last use ───────────────────────────────────
const daysSince = (d: string) => d ? Math.floor((Date.now() - +new Date(d)) / 86400000) : null
...
<style scoped>
.row       { @apply hover:bg-zinc-100 dark:hover:bg-zinc-900/40 cursor-pointer even:bg-zinc-50 dark:even:bg-zinc-900/20 relative; }

.cell-name { @apply px-1 py-0.5 truncate max-w-[300px]; }
.name-inner { @apply flex items-center gap-1; }
.name-link  { @apply truncate flex-1 text-3xs text-zinc-800 dark:text-zinc-100 hover:text-zinc-500 dark:hover:text-zinc-400 transition-colors; }
.note-dot   { @apply text-zinc-400 dark:text-zinc-700 text-3xs; }

.cell-buy  { @apply px-1 py-0.5 text-center; }
.cell-sym  { @apply text-2xs text-zinc-600 dark:text-zinc-500; }
.cell      { @apply px-1 py-0.5 text-center text-3xs text-zinc-500 dark:text-zinc-600; }
.cell-weight { @apply px-1 py-0.5 text-right text-zinc-700 dark:text-zinc-300 tabular-nums pr-2; }

.priority-pips { @apply tracking-tighter font-mono; }
.empty         { @apply text-zinc-400 dark:text-zinc-800 text-3xs; }
