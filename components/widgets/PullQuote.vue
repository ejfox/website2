...
<template>
  <div ref="el" class="pull-quote-section" :class="[`align-${align}`]">
    <blockquote class="pull-quote" :style="{ '--pq-accent': accent }">
      <p class="pull-quote-text">
        <span>{{ displayText }}</span>
        <span v-if="showCursor" class="pull-quote-cursor" />
      </p>
    </blockquote>
  </div>
</template>
...
.align-left .pull-quote {
  text-align: left;
}
.align-right .pull-quote {
  text-align: right;
  margin-left: auto !important;
}
.align-center .pull-quote {
  text-align: center;
  max-width: 36em;
