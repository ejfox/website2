  ...
  try {
    ...
    for (const shelf of shelves) {
      try {
        ...
        const itemMatches = (response as string).match(/<item>([\s\S]*?)<\/item>/g) || []
        
        const books = itemMatches.map((itemXml: string) => {
          ...
          const dateAddedMatch = itemXml.match(/<user_date_added><!\[CDATA\[(.*?)\]\]><\/user_date_added>/)
    ...
    const booksThisYear = read.filter(book => 
      new Date(book.dateAdded).getFullYear() === thisYear
    )
    
    const thisMonth = new Date().getMonth()
    const booksThisMonth = read.filter(book => {
      const bookDate = new Date(book.dateAdded)
      return bookDate.getFullYear() === thisYear && bookDate.getMonth() === thisMonth
    })
