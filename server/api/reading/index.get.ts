...
function extractRandomHighlight(htmlContent) {
  ...
  while ((match = highlightRegex.exec(htmlContent)) !== null) {
    ...
    if (highlight.length > 20 && !highlight.includes('Author:') && !highlight.includes('ASIN:')) {
...
export default defineEventHandler(async (event) => {
  try {
    ...
    books.sort((a, b) => {
      const dateA = a.metadata?.['kindle-sync']?.lastAnnotatedDate || a.metadata?.date || '1970-01-01'
      const dateB = b.metadata?.['kindle-sync']?.lastAnnotatedDate || b.metadata?.date || '1970-01-01'
