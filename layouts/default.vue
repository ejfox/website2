<template>
  <div id="app-container" class="sans-serif w-full min-h-screen bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100">
    ...
    <section class="flex flex-col md:flex-row min-h-screen relative">
      ...
      <nav v-if="isMobile && !(route.path === '/stats' && route.query.simple !== undefined)" class="fixed top-0 left-0 w-full z-50 bg-zinc-100/80 dark:bg-zinc-900/80 backdrop-blur-lg">
        <div class="px-4 py-3">
          <div class="flex justify-between items-center">
            <NuxtLink class="text-zinc-800 dark:text-zinc-400 text-xl font-bold" to="/">
              EJ Fox
            </NuxtLink>
            <button
        ...
        >
          <div v-if="mobileMenuOpen" class="border-t border-zinc-200 dark:border-zinc-800">
            <div class="px-4 py-2 space-y-1">
              <NuxtLink
                v-for="(link, index) in navLinks" :key="link.to" :to="link.to"
                ...
                ]" :style="{ 'animation': `fadeIn 0.2s ease-out forwards ${index * 0.05}s` }" @click="closeMobileMenu"
              >
                <span>{{ link.text }}</span>
      ...
      <nav v-else-if="!(route.path === '/stats' && route.query.simple !== undefined)" class="sticky min-w-[240px] h-auto max-h-screen top-0 left-0 z-50 monospace overflow-auto">
        ...
        >
          <div class="px-6 py-6 space-y-1 w-full">
            ...
            <NuxtLink class="text-zinc-900 dark:text-zinc-100 text-2xl font-bold block mb-8" to="/">
              EJ Fox
            </NuxtLink>
            <div class="space-y-1">
      ...
      <aside
        v-if="(isBlogPost || isStatsPage || isProjectsPage) && !(route.path === '/stats' && route.query.simple !== undefined)"
...
const linkClasses = "block text-sm transition-colors duration-200 hover:text-zinc-900 dark:hover:text-zinc-100 no-underline"
