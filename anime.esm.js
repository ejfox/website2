...
const createDrawable = (selector, start = 0, end = 0) => {
  const els = /** @type {Array.<Proxy>} */ (
    /** @type {unknown} */ (parseTargets(selector))
  )
  els.forEach(
    ($el, i) =>
      (els[i] = createDrawableProxy(
        /** @type {SVGGeometryElement} */ (/** @type {unknown} */ ($el)),
        start,
        end
      ))
  )
...
class Timer extends Clock {
  ...
  stretch(newDuration) {
    ...
    this.duration = isSetter
      ? minValue
      : clampZero(clampInfinity(round(currentDuration * timeScale, 12)))
    this.iterationDuration = isSetter
      ? minValue
      : clampZero(clampInfinity(round(this.iterationDuration * timeScale, 12)))
...
const easeInFunctions = {
  ...
  Elastic: (amplitude = 1, period = 0.3) => {
    ...
    const e = doublePI / p
    return (t) =>
      t === 0 || t === 1 ? t : -a * pow(2, -10 * (1 - t)) * sin((1 - t - s) * e)
  }
}
...
const parseEaseString = (string, easesFunctions, easesLookups) => {
  ...
  if (string.indexOf('(') <= -1) {
    const hasParams =
      easeTypes[string] || string.includes('Back') || string.includes('Elastic')
...
class JSAnimation extends Timer {
  ...
  stretch(newDuration) {
    ...
    forEachChildren(this, (/** @type {Tween} */ tween) => {
      // Rounding is necessary here to minimize floating point errors
      tween._updateDuration = clampZero(
        round(tween._updateDuration * timeScale, 12)
      )
      tween._changeDuration = clampZero(
        round(tween._changeDuration * timeScale, 12)
      )
