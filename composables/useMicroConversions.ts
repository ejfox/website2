...
export const FUNNEL_BENCHMARKS = {
  ...
  engaged_to_interested: 0.20, // 20% click to learn more
  interested_to_calendar: 0.15, // 15% visit calendar
  calendar_to_booking: 0.25, // 25% of calendar visitors book
  booking_to_meeting: 0.80, // 80% of bookings happen (no-show rate 20%)
  meeting_to_proposal: 0.60, // 60% get a proposal
  proposal_to_close: 0.30, // 30% close rate
}
...
export function useMicroConversions() {
  ...
  function trackMicro(conversion: MicroConversion) {
    ...
    if (import.meta.client && (window as unknown as { clarity?: (cmd: string, ...args: unknown[]) => void }).clarity) {
      (window as unknown as { clarity: (cmd: string, ...args: unknown[]) => void }).clarity('set', 'funnel_step', conversion.step.toString())
      ;(window as unknown as { clarity: (cmd: string, ...args: unknown[]) => void }).clarity('set', 'micro_conversion', conversion.name)
    }
  }
  ...
  const micro = {
    ...
    scrolledPastFold: () =>
      trackMicro({ name: 'scrolled_past_fold', step: 2 }),

    readFor30Seconds: () =>
      trackMicro({ name: 'read_30s', step: 2 }),

    readFor60Seconds: () =>
      trackMicro({ name: 'read_60s', step: 2 }),

    // Step 3: Interest (exploring offering)
    viewedPricing: () =>
      trackMicro({ name: 'viewed_pricing', step: 3 }),

    viewedCaseStudies: () =>
      trackMicro({ name: 'viewed_case_studies', step: 3 }),

    viewedProcess: () =>
      trackMicro({ name: 'viewed_process', step: 3 }),
    ...
    viewedCalendar: () =>
      trackMicro({ name: 'viewed_calendar', step: 4 }),

    calendarLoaded: () =>
      trackMicro({ name: 'calendar_loaded', step: 4 }),

    // Step 5: Intent (interacting with calendar)
    selectedDate: () =>
      trackMicro({ name: 'selected_date', step: 5 }),

    selectedTime: () =>
      trackMicro({ name: 'selected_time', step: 5 }),

    startedForm: () =>
      trackMicro({ name: 'started_form', step: 5 }),

    // Step 6: Conversion (booking complete)
    completedBooking: () =>
      trackMicro({ name: 'completed_booking', step: 6 }),
    ...
    addedToCalendar: () =>
      trackMicro({ name: 'added_to_calendar', step: 7 }),
  }
