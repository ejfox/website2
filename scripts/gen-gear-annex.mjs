...
const rows = d3.csvParse(await readFile(CSV, 'utf-8')).filter((r) => r.Name?.trim())
const active = rows.filter((r) => !['Archived', 'HomeBase'].includes((r.Status || '').trim()))

const tagsOf = (r) => (r.Tags || '').toLowerCase().split(';').map((s) => s.trim())
const has = (r, t) => tagsOf(r).includes(t)
const yes = (r, f) => (r[f] || '').trim().toLowerCase() === 'yes'

const BUCKETS = [
  ['charge', 'Charge everything that takes a charge', (r) => yes(r, 'Powered') || has(r, 'powered')],
  ['capture', 'Clear / format every capture device storage', (r) => yes(r, 'HasStorage') || has(r, 'capture')],
  ['riding', 'Full ATGATT — don before rolling', (r) => has(r, 'riding') || has(r, 'atgatt')],
  ['waterproof', 'Waterproof / shell — keep accessible', (r) => yes(r, 'Waterproof')],
  ['sleep', 'Sleep system — layer to the forecast low', (r) => (r.Type || '').trim() === 'Sleep'],
  ['restock', 'Restock consumables (food / water / fuel)', (r) => yes(r, 'Consumable')],
]
...
for (const [anchor, desc, pred] of BUCKETS) {
  const items = active.filter(pred).map((r) => r.Name.trim()).sort((a, b) => a.localeCompare(b))
  md += `## ${anchor}\n*${desc}*\n\n`
  md += items.length ? items.map((n) => `- [ ] ${n}`).join('\n') : '_(none tagged yet)_'
  md += `\n\n`
}

// Charging audit — group powered items by connector so you know which cables/bricks to bring.
const poweredItems = active.filter((r) => yes(r, 'Powered') || has(r, 'powered'))
const byPort = d3.group(poweredItems, (r) => (r.ChargePort || '').trim() || '(unknown — fill in)')
...
for (const port of [...byPort.keys()].sort()) {
  const items = byPort.get(port).map((r) => r.Name.trim()).sort((a, b) => a.localeCompare(b))
  md += `**${port}** (${items.length}): ${items.join(', ')}\n\n`
}

// Card audit — group capture devices by media type so you bring the right cards + readers.
const captureItems = active.filter((r) => yes(r, 'HasStorage') || has(r, 'capture'))
const byCard = d3.group(captureItems, (r) => (r.CardType || '').trim() || '(unknown — fill in)')
...
for (const card of [...byCard.keys()].sort()) {
  const items = byCard.get(card).map((r) => r.Name.trim()).sort((a, b) => a.localeCompare(b))
  md += `**${card}** (${items.length}): ${items.join(', ')}\n\n`
}

const untagged = active.filter((r) => (r.Type || '').trim() === 'Tech' && !has(r, 'powered'))
if (untagged.length) {
...
for (const [a, , pred] of BUCKETS) console.log(`  ${a}: ${active.filter(pred).length}`)
