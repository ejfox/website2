...
function findThirdPartyReferences(dir, results = []) {
  ...
  for (const file of files) {
    ...
    if (file.isDirectory()) {
      ...
      if (
        ['node_modules', '.git', '.nuxt', 'dist', '.output'].includes(file.name)
      ) {
        continue
      }
...
function generateSecurityReport(results) {
  ...
  for (const level of ['HIGH', 'MEDIUM', 'LOW', 'UNKNOWN']) {
    report.summary.byRiskLevel[level] = results.filter(
      (r) => r.riskLevel === level
    ).length
  }
  ...
  for (const category of Object.keys(AUDIT_PATTERNS)) {
    report.summary.byCategory[category] = results.filter(
  ...
  if (report.summary.byRiskLevel.HIGH > 0) {
    report.recommendations.push(
      '⚠️ HIGH RISK: Remove or replace high-risk third-party domains'
    )
  }
  ...
  if (domains.length > 0) {
    ...
    domains.sort().forEach((domain) => {
      const domainFindings = report.findings.filter(
        (f) => new URL(f.url).hostname === domain
      )
