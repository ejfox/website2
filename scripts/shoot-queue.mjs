...
const flag = (name, def = false) => { const i = argv.indexOf('--' + name); return i === -1 ? def : (argv[i + 1] && !argv[i + 1].startsWith('--') ? argv[i + 1] : true) }
const TARGET = parseInt(flag('target', '4'), 10)
const MIN = parseInt(flag('min', String(TARGET)), 10)   // queue projects with < MIN images
const ONLY = typeof flag('only') === 'string' ? flag('only').split(',').map(s => s.trim()) : null
...
const bold = s => c(1, s), dim = s => c(2, s), green = s => c(32, s), cyan = s => c(36, s), yellow = s => c(33, s), red = s => c(31, s)
...
function parse(file) {
  ...
  return { slug, file, text, fm, body, images, count: images.length, desc: firstProse(body) }
}
...
// ---- queue -----------------------------------------------------------------
function buildQueue() {
  let files = readdirSync(PROJ_DIR).filter(f => f.endsWith('.md') && !f.startsWith('!'))
...
// ---- helpers ---------------------------------------------------------------
const sh = (cmd, args, opts = {}) => spawnSync(cmd, args, { stdio: 'inherit', cwd: ROOT, ...opts })
function openThing(target) { if (!NO_OPEN) spawn('open', [target], { stdio: 'ignore', detached: true }).unref() }
function hasCreds() { return !!(process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) }
...
function seed(slug, url, drop) {
  ...
  sh('node', ['scripts/capture.mjs', 'scroll', url, join(drop, 'seed'), '--count', '5', '--wait', '1400', '--size', '1440,900'],
     { stdio: ['ignore', 'ignore', 'inherit'] })
}

// Upload one local file -> projects/<slug>/<id>, return the cloudinary URL (or null).
function upload(localFile, publicId) {
  if (DRY) return `https://res.cloudinary.com/${CLOUD}/image/upload/${publicId}` + extOf(localFile)
  const r = spawnSync('node', ['scripts/capture.mjs', 'upload', localFile, publicId], { cwd: ROOT, encoding: 'utf8' })
  const out = (r.stdout || '') + (r.stderr || '')
  const m = out.match(/https:\/\/res\.cloudinary\.com\/\S+/)
  if (!m) { console.log(red('  upload failed: ') + out.trim().split('\n').pop()); return null }
  return m[0].replace(/\.png$/, extOf(localFile))
}
const extOf = f => (f.match(/\.(png|jpg|jpeg|gif|webp)$/i) || ['.png'])[0].toLowerCase()
const caption = f => f.replace(/\.[^.]+$/, '').replace(/^seed_?/, '').replace(/[-_]+/g, ' ').replace(/\d+$/, '').trim() || 'screenshot'
...
async function run() {
  ...
  if (!hasCreds() && !DRY) console.log(yellow('⚠  No Cloudinary creds in .env — uploads will fail. Add CLOUDINARY_* or run --dry-run.\n'))
  if (DRY) console.log(dim('   DRY RUN — capturing + previewing markdown, nothing uploaded or written.\n'))
  if (!queue.length) { console.log(green('Nothing to do — every project meets the target. 🎉')); rl.close(); return }
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    console.log('\n' + dim('─'.repeat(64)))
    console.log(`${bold(`[${i + 1}/${queue.length}]`)} ${cyan(bold(p.fm.title || p.slug))}  ${dim(p.slug)}`)
    console.log(`  ${dim(p.desc)}`)
    console.log(`  images: ${p.count < TARGET ? red(p.count) : green(p.count)}/${TARGET}   ${p.fm.url ? dim(p.fm.url) : dim('(no url)')}`)
    ...
    do {
      ...
      if (answer === 'o') { if (p.fm.url) openThing(p.fm.url); openThing(drop) }
    } while (answer === 'o')
    if (answer === 'q') break
    if (answer === 's') { console.log(dim('  skipped.')); continue }

    const shots = readdirSync(drop).filter(f => /\.(png|jpe?g|gif|webp)$/i.test(f) && !f.startsWith('.')).sort()
    if (!shots.length) { console.log(yellow('  no images in the drop folder — skipping.')); continue }
    ...
    if (!newLines.length) { console.log(yellow('  nothing wired.')); continue }
    console.log(dim('\n  markdown to add:\n') + newLines.map(l => '    ' + l).join('\n'))
    if (DRY) { console.log(dim('  (dry-run — not writing .md)')); continue }
    wire(p, newLines)
    console.log(green(`  ✓ wired ${newLines.length} image${newLines.length === 1 ? '' : 's'} into ${p.slug}.md`) + dim(`  (backup: ${p.slug}.md.bak)`))
    done++; added += newLines.length
  }

  console.log('\n' + bold(`done — ${done} project${done === 1 ? '' : 's'} updated, ${added} image${added === 1 ? '' : 's'} added.`))
  if (added && !DRY) console.log(dim('next: yarn blog:process && review, then commit. .bak files left next to each edited .md.'))
  rl.close()
}
run().catch(e => { console.error(red(e.stack || e.message)); rl.close(); process.exit(1) })
