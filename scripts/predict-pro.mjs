...
function getScoreColor(score) {
  switch(score) {
    ...
    case 'needs_improvement': return chalk.yellow('●')
...
const validateDate = (dateStr) => {
  ...
  if (date <= oneWeek) return { valid: false, reason: "Deadline must be at least one week in the future" }
  if (date.getFullYear() > 2050) return { valid: false, reason: "Deadline too far in the future (max 2050)" }
...
const validateConfidence = (conf) => {
  ...
  if (num < 5 || num > 95) return { valid: false, reason: "Confidence should be between 5-95% (avoid overconfidence)" }
  return { valid: true }
}

const validateStatement = (statement) => {
  if (!statement || statement.length < 20) {
    return { valid: false, reason: "Statement too short - be more specific (min 20 chars)" }
  }
  if (statement.length > 300) {
    return { valid: false, reason: "Statement too long - break into multiple predictions (max 300 chars)" }
  }
  
  // Check for common issues
  const issues = []
  if (!/\d/.test(statement)) issues.push("Consider adding specific numbers or dates")
  if (/maybe|might|could|possibly/i.test(statement)) issues.push("Avoid uncertainty words - state what will happen")
...
async function checkPredictionQuality(statement, resolutionCriteria) {
  ...
  if (!openRouterKey) {
    log.info('Using built-in quality analysis (AI analysis available with OPENROUTER_API_KEY)')
...
function getBuiltInAnalysis(statement, resolutionCriteria) {
  const analysis = {
    ...
    measurability: resolutionCriteria.length > 20 ? 'good' : 'needs_improvement',
    resolvability: resolutionCriteria.length > 50 ? 'good' : 'needs_improvement',
    overallScore: 7,
    suggestions: [],
    strengths: [],
    concerns: []
  ...
  if (/\$|\%/.test(statement)) analysis.strengths.push("Includes specific numerical targets")
...
function showHelp() {
  ...
  console.log('  yarn predict                                    Interactive wizard')
  console.log('  yarn predict --help                             Show this help')
  ...
  console.log(chalk.gray('    --statement "Bitcoin will exceed $150,000 by Dec 31, 2025" \\'))
  console.log(chalk.gray('    --confidence 75 \\'))
  console.log(chalk.gray('    --deadline 2025-12-31 \\'))
  console.log(chalk.gray('    --resolution "Using CoinGecko closing price at 11:59 PM UTC" \\'))
...
async function createPredictionFromArgs(args) {
  ...
  const errors = []
  if (!validation.statement.valid) errors.push(`Statement: ${validation.statement.reason}`)
  if (!validation.confidence.valid) errors.push(`Confidence: ${validation.confidence.reason}`)
  if (args.deadline && !validation.deadline.valid) errors.push(`Deadline: ${validation.deadline.reason}`)
  ...
  // Create prediction
  const id = createHash('sha256').update(args.statement + Date.now()).digest('hex').substring(0, 8)
...
async function enhancedPredictWizard() {
  ...
  console.log(chalk.gray('   Create high-quality, cryptographically verifiable predictions\n'))
  
  log.info('This wizard will guide you through creating well-formatted predictions')
  log.info('We\'ll help ensure your prediction is specific, measurable, and resolvable')
  ...
  while (!resolutionCriteria || resolutionCriteria.length < 20) {
    resolutionCriteria = await question(chalk.bold('\n⚖️  How exactly will you determine if this was correct?\n   (Include data sources, edge cases, measurement details): '))
    if (!resolutionCriteria || resolutionCriteria.length < 20) {
      log.warning('Please provide more detailed resolution criteria (min 20 characters)')
      log.tip('Example: "Using CoinGecko closing price on December 31, 2025 at 11:59 PM UTC"')
    }
  }
  ...
  if (analysis.overallScore) {
    ...
    console.log(chalk.bold('\n   📊 Detailed Scores:'))
    console.log(`   Clarity: ${getScoreColor(analysis.clarity)} ${analysis.clarity}`)
    console.log(`   Specificity: ${getScoreColor(analysis.specificity)} ${analysis.specificity}`)
    console.log(`   Measurability: ${getScoreColor(analysis.measurability)} ${analysis.measurability}`)
    console.log(`   Resolvability: ${getScoreColor(analysis.resolvability)} ${analysis.resolvability}`)
  }
  ...
  while (!validateConfidence(confidence).valid) {
    confidence = await question(chalk.bold('\n🎯 How confident are you? (5-95): '))
  ...
  while (!validateDate(deadline).valid) {
    deadline = await question(chalk.bold('\n📅 Resolution deadline (YYYY-MM-DD): '))
  ...
  log.info('Common categories: ai, economics, politics, technology, climate, sports')
  const categoriesInput = await question(chalk.bold('\n🏷️  Categories (comma-separated): '))
  const categories = categoriesInput ? 
    categoriesInput.split(',').map(c => c.trim().toLowerCase()).filter(c => c) : []
  ...
  console.log(chalk.gray('   Press Enter twice when done, or skip with double Enter.\n'))
  ...
  console.log(chalk.bold('Categories:'), chalk.white(categories.join(', ') || 'none'))
  
  const confirm = await question(chalk.bold('\n✅ Create this prediction? (Y/n): '))
  ...
  const id = createHash('sha256').update(statement + Date.now()).digest('hex').substring(0, 8)
  const created = new Date().toISOString()
...
${evidence.trim() ? `\n# Evidence and Reasoning\n\n${evidence.trim()}` : ''}`
  ...
  log.success('Your prediction is cryptographically verified and ready for tracking!')
}

// Utility functions (same as before)
function generateFilename(statement, deadline) {
  const year = deadline ? new Date(deadline).getFullYear() : new Date().getFullYear()
