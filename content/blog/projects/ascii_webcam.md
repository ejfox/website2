---
title: "ascii_webcam"
date: 2024-10-01T00:00:00-04:00
draft: true
url: https://github.com/ejfox/ascii_webcam
tech: ["Python", "ASCII Art", "Terminal", "OpenCV"]
state: deployed
aiInvolvement: ai-assisted
tags:
  - cli
  - terminal
  - programming
---

<!-- TODO (EJ): split out from the old "Terminal Tools" roundup — add your voice.
     The GitHub README has .mov demo clips worth embedding once the filmstrip
     supports video. Draft until you fill this in. -->

Show your webcam as ASCII art in the terminal — customizable cyberpunk 3D scenes, gradient palettes, and Floyd–Steinberg dithering.

![ascii_webcam rendering a cyberpunk 3D ASCII scene in the terminal](https://res.cloudinary.com/ejf/image/upload/v1743884268/Screenshot_2025-04-05_at_4.17.34_PM.png)
