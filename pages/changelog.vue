<template>
  <div class="max-w-screen-xl mx-auto px-4 md:px-8 py-8">
    <div class="max-w-4xl">
      ...
      <!-- Stats -->
      <section v-if="data" class="mb-8 grid grid-cols-2 md:grid-cols-4 gap-4 font-mono text-xs">
        <div v-for="(count, type) in data.stats.byType" :key="type"
             class="border-l-2 border-zinc-300 dark:border-zinc-700 pl-2">
          <div class="text-zinc-900 dark:text-zinc-100 font-bold">{{ count }}</div>
          <div class="text-zinc-500 dark:text-zinc-500">{{ type }}</div>
        </div>
      </section>

      <!-- Timeline -->
      <section v-if="data">
        <div v-for="(commits, date) in data.grouped.byDate" :key="date" class="mb-6">
          <h2 class="font-mono text-xs text-zinc-500 dark:text-zinc-500 uppercase tracking-wider mb-2 sticky top-0 bg-white dark:bg-zinc-950 py-1">
            {{ formatDate(date) }}
          </h2>
          ...
          <div class="space-y-2">
            <div v-for="commit in commits" :key="commit.hash"
              ...
              <p class="font-mono text-xs text-zinc-400 dark:text-zinc-600 mt-1">
      ...
      <footer class="mt-8 text-center">
        <button
          v-if="canLoadMore"
          @click="loadMore"
          class="font-mono text-xs text-zinc-500 dark:text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
        >
          Load more commits
        </button>
      </footer>
      ...
      <footer class="mt-12 pt-4 border-t border-zinc-200 dark:border-zinc-800">
        <p class="font-mono text-xs text-zinc-500 dark:text-zinc-500">
          ...
          <a href="/api/changelog" class="hover:text-zinc-900 dark:hover:text-zinc-100">JSON</a>
        </p>
      </footer>
    </div>
  </div>
...
const canLoadMore = computed(() => data.value && data.value.commits.length >= limit.value)
...
function getTypeColor(type: string) {
  ...
  return colors[type] || 'bg-zinc-100 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100'
}
