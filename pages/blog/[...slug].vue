...
function formatDate(date) {
  ...
  let parsedDate = typeof date === 'string' ? parseISO(date) : date instanceof Date ? date : null
  return !parsedDate || !isValid(parsedDate) ? 'Invalid date' : format(parsedDate, 'MMMM d, yyyy')
}

function getBaseUrl() {
  if (config.public && config.public.baseURL) return config.public.baseURL
  return typeof window !== 'undefined' ? window.location.origin : 'https://ejfox.com'
}

const baseURL = getBaseUrl()
const shareImageUrl = new URL(`/images/share/${params.slug.join('/')}.png`, baseURL).href
...
<template>
  <div>
    <article v-if="post" class="scroll-container pt-10 md:pt-4">
      ...
      <div ref="postContent">
        <article v-html="post.content" class="
        px-2 prose-lg md:prose-xl dark:prose-invert 
        ...
        prose-code:overflow-x-auto prose-code:max-w-full
        !max-w-none font-weight-normal">
        </article>
      </div>
